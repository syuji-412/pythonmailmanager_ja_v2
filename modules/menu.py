...
def main_menu():
    ...
    print('   3. 終了')
    print()
    menu = input(color.pycolor.CYAN + ' メニュー番号を選択: ' + color.pycolor.END)
...
def send_from_tsv():
    ...
    while os.path.isfile(tsv_file) == False:
        tsv_file = input(color.pycolor.CYAN + ' TSVファイルを指定（指定しない場合は "send.tsv" または "q" でTSVから送信を中止）: ' + color.pycolor.END)
...
def send_indivisually():
    ...
    specified_domains = input(color.pycolor.CYAN + ' カンマ区切りで複数のドメインを指定可能: ' + color.pycolor.END)
    ...
    for domain in specified_domains.split(','):
        ...
        mail_from = input(' 送信元アドレス（デフォルトは "' + config['send_default_account'][domain].split(',')[0].split(':')[0] + '"）: ')
        ...
        rcpt_to = input(' 送信先アドレス（デフォルトは "' + config['send_default_account'][domain].split(',')[1] + '"）: ')
...
def pop_from_tsv(): 
    ...
    tsv_file = ''
    while os.path.isfile(tsv_file) == False:
        tsv_file = input(color.pycolor.CYAN + ' TSVファイルを指定（指定しない場合は "pop.tsv" または "q" TSVから受信を中止）: ' + color.pycolor.END)
...
def pop_indivisually():
    ...
    for domain in specified_domains.split(','):
        print(color.pycolor.GREEN + ' ['+domain+']'+ color.pycolor.END)
        username = input(' ユーザー名(デフォルトは "' + config['pop_default_account'][domain].split(':')[0] + '"): ')
